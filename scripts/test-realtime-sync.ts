...
export async function testRealTimeSync() {
  ...
  console.log(`   전체 단계 수: ${enhancedRealtimeService.getTotalSteps()}`);
  ...
  const updatedQuestions = enhancedRealtimeService.getQuestions();
  console.log(`   첫 번째 질문: ${updatedQuestions[0].question.substring(0, 50)}...`);

  console.log('\n7️⃣ 원래 상태로 복원');
  await enhancedRealtimeService.saveQuestions(currentQuestions);
  console.log('   복원 완료');
